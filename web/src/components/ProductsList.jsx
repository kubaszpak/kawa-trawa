...
import { Typography, Select, MenuItem, Paper, Box, FilledInput } from "@mui/material";
    ...
    return (
        <>
            <Box className={classes.grid} >
                <Grid container rowSpacing={2} columnSpacing={2} flexDirection={"row-reverse"} >
                    <Grid item>
                ...
                <Grid container rowSpacing={2} columnSpacing={2} >
                    {displayedProducts.map((product, index) => (
                        <Grid item xs={12} sm={6} md={4} xl={3} key={index}>
                            <ProductCard key={index} product={product} addProductToCart={addProductToCart} onDelete={onDelete} />
                        </Grid>
                    ))}
                </Grid>
            </Box>
